...
public class Cardinal extends JavaPlugin {
    ...
    private void setupCommands() {
        ...
        cmdRegister.register(TournamentCommands.TournamentParentCommand.class);
    }
    ...
    public void onEnable() {
        ...
        if (databaseFile.exists()) {
            try {
                ...
                e.printStackTrace();
                Bukkit.getLogger().log(Level.INFO, "[AntiquityPGM] Failed to initialize because of an IOException, \n Please try restarting your server.");
                this.setEnabled(false);
                return;
            }
        } else {
        ...
        if (config.getBoolean("deleteMatches")) {
            Bukkit.getLogger().log(Level.INFO, "[AntiquityPGM] Deleting match files, \n this can be disabled via the configuration.");
        ...
        try {
            ...
            Bukkit.getLogger().log(Level.SEVERE, "[AntiquityPGM] Failed to initialize because of an invalid rotation configuration.");
            setEnabled(false);
            return;
        }
        setupCommands();
